 ...
 * Los registros existentes con otros nombres (p. ej. ADMINISTRADOR, Alison) se
 * conservan.
 */
@Service
public class Phase1MigrationService {
    ...
    public void seedIfEmpty() {
        // Si no hay roles, los creamos
        if (roleRepository.count() == 0) {
            migrateRoles(new MigrationReport());
        }
        // Si no hay departamentos, los creamos
        if (departmentRepository.count() == 0) {
            migrateDepartments(new MigrationReport());
        }

        // CAMBIO AQUÍ: En lugar de solo contar usuarios, verificamos si el admin
        // específico existe
        Optional<User> admin = userRepository.findByUsername("sistema.admin");
        if (admin.isEmpty()) {
            log.info("Usuario administrador no encontrado, forzando migración de usuarios...");
    ...
    private void migrateUsers(MigrationReport report) {
        for (UserSeed seed : Phase1BootstrapData.USERS) {
            ...
            if (department == null) {
                report.addWarning(
                        "Departamento no encontrado para usuario " + seed.username() + ": " + seed.departmentName());
            }
